...
def load_bands(fPath, 
    ...
    """
      :descr:
        Initially made to transform all the bands of a satellite image, contained in differents files in the same folder in a numpy.array object.
        You can specify an area with a list of coordinates or with a shapefile to load the bands only on those area.
    
    :param:
        fPath (str) = name of the folder where the images files are stored
        pattern (str) = a regular expression pattern corresponding to all the files you want to load. If a string match, it will be the key to find the array in the dictionnary returned.
        endKeyPos (int) = help you to build keys easy to understand. By default -4, just to exclude the extension of the file. (ex: "B02.tif" > "B02")
        ...
        verbose (boolean) = if it's True, the function give informations during is execution. It's interesting for debugging.

    :returns:
        bands (dictionnary) = contain all the file with a name where the pattern match, organised with the str combinaison which match the pattern.
